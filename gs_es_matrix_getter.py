...
#delta-dft output files
gs = sys.argv[1]
es = sys.argv[2]

#useful parameters to extract from either output file
...
#begin iterating through gs file
for i, gline in enumerate(glines):

	if gline.find("basis functions") != -1: #extract nbas
		nbas = int(gline.strip().split()[5])

	if gline.find("beta electrons") != -1: #extract number of electrons/2
		n_a_elec = int(gline.strip().split()[2])

	if copy_o > 0 and gline.find("Core Hamiltonian") != -1: #end copying of overlap matrix
		copy_o = 0

	if copy_o > 0: #copy overlap matrix while flag is on
		#print(line)
		o_mat.append(gline.strip().split())

	if gline.find("Overlap Matrix") != -1: #begin copying overlap matrix on the next line
		copy_o = 1

	#if line.find("Have a nice day") != -1: #switch to job 2
		#job = 2

#begin iterating through es file		
for i, eline in enumerate(elines):
	if copy_c > 0 and eline.find("Ground-State Mulliken Net Atomic Charges")!= -1: #stop copying MO coeficient matrix
		copy_c = 0

	if copy_c > 0: #copy MO coefficient matrix while flag is on
		#print(line)
		coeff.append(eline.strip().split())

	if eline.find("ALPHA MOLECULAR ORBITAL COEFFICIENTS")!= -1: #begin copying MO coefficient matrix on next line
		copy_c = 1


#prune saved arrays so that they only contain desired matrix elements

npop = math.ceil(nbas/6) #QChem outputs the Overlap matrix 6 columns at a time. Npop is the number of times it does this
...
final_o_mat =np.array(proper_o_mat)
np.savez("o_mat.npz",final_o_mat)
...
n_a_pop = math.ceil((nbas)/6) #just like before, qchem prints 6 MOs per row

for i in range(n_a_pop): #this deletes the rows that contain the MO indices and MO energies
	#print(coeff[(n_a_pop-i-1)*(nbas+2)+1])
	#print(coeff[(n_a_pop-i-1)*(nbas+2)])
	coeff.pop((n_a_pop-i-1)*(nbas+2)+1)
	coeff.pop((n_a_pop-i-1)*(nbas+2))
#print(coeff)

#delete the last 3 rows of the alpha section, including "BETA MOLECULAR ORBITAL COEFFICIENTS" 
#print(coeff[nbas*(n_a_pop)])
coeff.pop(nbas*(n_a_pop))
#print(coeff[nbas*(n_a_pop)])
coeff.pop(nbas*(n_a_pop))
#print(coeff[nbas*(n_a_pop)])
coeff.pop(nbas*(n_a_pop))
...
n_b_pop = math.ceil((nbas)/6)

#print(coeff[nbas*n_a_pop])
for i in range(n_b_pop): #again, remove eigenvalues and indices
	#print(coeff[nbas*(n_a_pop)+(n_a_pop-i-1)*(nbas+2)+1])
	coeff.pop(nbas*(n_a_pop)+(n_a_pop-i-1)*(nbas+2)+1)
	#print(coeff[nbas*(n_a_pop)+(n_a_pop-i-1)*(nbas+2)])
	coeff.pop(nbas*(n_a_pop)+(n_a_pop-i-1)*(nbas+2))
